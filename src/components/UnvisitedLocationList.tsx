...
export default function UnvisitedLocation({ groupId, locations }: Props) {
    ...
    return (
        <div className="mb-10 md:mb-15 bg-white rounded-md p-2">
            <h2 className="text-lg md:text-xl font-bold mb-3 md:mb-8">行きたい場所</h2>
            
            {locations.length === 0 ? (
                <p className="text-gray-500 text-xs md:text-sm mb-5 ml-2">まだ行きたい場所が追加されていません。</p>
            ) : (
                <ul>
                {locations.map((location) => (
                    <li key={location.id} className="mb-3 md:mb-5 border-b border-emerald-500 pb-2 md:pb-3">
